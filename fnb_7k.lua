...
RunService.Heartbeat:Connect(function()
    ...
    local arrowsaresus = SideMenu.Arrows.IncomingNotes
    local IncomingArrows = SideMenu.Arrows.IncomingNotes

    local Keys = KeysTable[tostring(#arrowsaresus:GetChildren())] or arrowsaresus:GetChildren()

    for Key, Direction in pairs(Keys) do
        Direction = tostring(Direction)

        local ArrowsHolder = arrowsaresus:FindFirstChild(Direction) or arrowsaresus:FindFirstChild(Key)
        ...
        for _, Object in ipairs(ArrowsHolder:GetChildren()) do
            ...
            if Difference <= 10 and not IsHell then
...
do
    local Window = Library:CreateWindow("Friday Night Bloxxin'") do
        ...
        Folder:AddLabel({text = "It works better now, but it is"})
        Folder:AddLabel({text = "not perfect enough..."})
        Folder:AddLabel({text = "======================"})
        Folder:AddLabel({text = "UnPatched by Veinhel"})
        Folder:AddLabel({text = "======================"})
        Folder:AddLabel({text = "Fatitious is sus"})
        Folder:AddLabel({text = "======================"})
        Folder:AddLabel({text = "7k version"})
        Folder:AddLabel({text = "======================"})
